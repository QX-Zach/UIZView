package personal.ui.lingchen.uizview.UI.bubbleseekbar;
...
import static personal.ui.lingchen.uizview.UI.bubbleseekbar.BubbleSeekBar.TextPosition.BELOW_SECTION_MARK;
import static personal.ui.lingchen.uizview.UI.bubbleseekbar.BubbleSeekBar.TextPosition.BOTTOM_SIDES;
import static personal.ui.lingchen.uizview.UI.bubbleseekbar.BubbleSeekBar.TextPosition.SIDES;
import static personal.ui.lingchen.uizview.UI.bubbleseekbar.BubbleUtils.dp2px;
import static personal.ui.lingchen.uizview.UI.bubbleseekbar.BubbleUtils.sp2px;
...
public class BubbleSeekBar extends View {
    ...
    private int trackYOffset, trackXOffset;
    ...
    public BubbleSeekBar(Context context, AttributeSet attrs, int defStyleAttr) {
        ...
        mSuffix = a.getString(R.styleable.BubbleSeekBar_bsb_suffix);
        a.recycle();
        ...
        mRectText = new Rect();
        mRectBubbleText = new Rect();

        mTextSpace = dp2px(2);
        trackYOffset = dp2px(25);

        mPaint.setTextSize(mBubbleTextSize);
        mPaint.getTextBounds(mSuffix, 0, mSuffix.length(), mRectBubbleText);
        Log.e("lingchen", "BubbleSeekBar:单位：" + mSuffix);
        trackXOffset = mRectBubbleText.width() / 2;

        mPaint.setTextSize(mBubbleTextSize);
        mPaint.getTextBounds(getMaxText(), 0, getMaxText().length(), mRectBubbleText);
...
//                                            String.valueOf(getProgressFloat()) : String.valueOf(getProgress()));
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        int height = mThumbRadiusOnDragging * 2 + trackYOffset + dp2px(2); // 默认高度为拖动时thumb圆的直径
        if (isShowThumbText) {
        ...
        mTrackLength = mRight - mLeft - trackXOffset * 2;
...
//        mBubbleCenterRawX = mBubbleCenterRawSolidX + mTrackLength * (mProgress - mMin) / mDelta;
        mBubbleCenterRawX = mTrackLength * (mProgress - mMin) / mDelta;
...
//        }
    ...
    protected void onDraw(Canvas canvas) {
        ...
        float yTop = getPaddingTop() + mThumbRadiusOnDragging + trackYOffset;
        ...
        sb.append(isShowProgressInFloat ? String.valueOf(getProgressFloat()) : String.valueOf(getProgress()));
        sb.append(mSuffix);
        float bubbleStart = mBubbleCenterRawX + mRectBubbleText.width() / 2 + trackXOffset;
        canvas.drawText(sb.toString(), bubbleStart, trackYOffset / 2, mPaint);
...
//        canvas.drawCircle(mThumbCenterX, yTop, isThumbOnDragging ? mThumbRadiusOnDragging : mThumbRadius, mPaint);
        int innerRadius = isThumbOnDragging ? mThumbRadiusOnDragging : mThumbRadius;
        canvas.drawCircle(mThumbCenterX, yTop, innerRadius + dp2px(2), mPaint);
        mPaint.setColor(0xffffffff);
        canvas.drawCircle(mThumbCenterX, yTop, innerRadius, mPaint);
        ...
        switch (event.getActionMasked()) {
            case MotionEvent.ACTION_DOWN:
                ...
                if (isThumbOnDragging) {
                    ...
                    if (mThumbCenterX > mRight - trackXOffset) {
                        mThumbCenterX = mRight - trackXOffset;
                    }
...
//        ObjectAnimator alphaAnim = ObjectAnimator.ofFloat(mBubbleView, View.ALPHA, isAlwaysShowBubble ? 1 : 0);
        ObjectAnimator alphaAnim = ObjectAnimator.ofFloat(this, View.ALPHA, 1);
